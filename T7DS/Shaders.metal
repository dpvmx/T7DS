...
struct Uniforms {
    float mode;             // 0:Bayer, 1:CRT, 2:Blue, 3:Diff, 4:Half
    float isGrayscale;
    float brightness;
    float contrast;
    float saturation;
    ...
    float crtGlowSpread;
    float halftoneMisprint; // Restaurado y funcional
};
...
float halftone(float2 uv, float intensity, float scale, float2 res, float ditherSpread) {
    ...
    float2 rotatedST = float2(st.x * c - st.y * s, st.x * s + st.y * c);
    float frequency = 150.0 / max(0.1, scale);
    float2 p = fract(rotatedST * frequency) - 0.5;
    float radius = intensity * (0.6 * ditherSpread);
    return smoothstep(radius, radius - 0.15, length(p));
}
...
float getBayer16(int x, int y) { int x8 = x % 8, y8 = y % 8, x2 = (x/8)%2, y2 = (y/8)%2; return (bayer8[y8][x8] * 4.0 + bayer2[y2][x2]) / 4.0; }
                        ...
                        uint2 gid [[thread_position_in_grid]])
{
    ...
    if (uniforms.mode > 3.5) {
        // --- MODO 4: HALFTONE CON MISPRINT ---
        float preBlur = uniforms.bayerScale * 0.4;
        float3 rgb = adjustColor(sampleHQ(inputTexture, s, uv, res, preBlur), uniforms);
        
        if (uniforms.isGrayscale > 0.5) {
            float pat = halftone(uv, dot(rgb, float3(0.299, 0.587, 0.114)), uniforms.bayerScale, res, uniforms.spread);
            outColor = mix(uniforms.colorDark.rgb, uniforms.colorLight.rgb, pat);
        } else {
            // Lógica Misprint CMYK Shift
            float mis = uniforms.halftoneMisprint * 0.015;
            outColor = float3(
                halftone(uv + float2(-mis, -mis), rgb.r, uniforms.bayerScale, res, uniforms.spread),
                halftone(uv, rgb.g, uniforms.bayerScale, res, uniforms.spread),
                halftone(uv + float2(mis, mis), rgb.b, uniforms.bayerScale, res, uniforms.spread)
            );
        }
    }
        ...
        float slotW = 3.0 * uniforms.crtScale; float slotH = 6.0 * uniforms.crtScale;
        float3 mask = float3(0.5 + 0.5 * cos((wuv.x * w + hash(uv)*uniforms.crtBlur) * (2.0 * M_PI_F / slotW) + float3(0, 2.1, 4.2)));
        mask *= mix(1.0, 0.5 + 0.5 * sin((wuv.y * h + hash(uv)*uniforms.crtBlur) * (2.0 * M_PI_F / slotH)), uniforms.crtScanline);
        ...
        // --- MODO 0, 2, 3: DIGITAL (Bayer, Blue, Diff) ---
        float2 mUV = floor(float2(gid) / uniforms.bayerScale) * uniforms.bayerScale;
        float3 rgb = adjustColor(inputTexture.read(uint2(mUV)).rgb, uniforms);
        float n = 0;
        int sx = int(floor(float(gid.x) / uniforms.bayerScale)), sy = int(floor(float(gid.y) / uniforms.bayerScale));
        ...
        } else if (uniforms.mode > 2.5) n = fract(hash(float2(sx, sy)) + (float(sx + sy) * 0.33));
        else n = hash(float2(sx, sy));
